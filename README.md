...
A browser arena-survival game built entirely from Pixel Frog's **Tiny Swords (Free Pack)**.
Hold a small island against an escalating horde for ten minutes, chain sword swings through crowds, dash out of lancer charges, level up and pick upgrades. Vanilla Canvas 2D, no build step, no dependencies, no downloads.
...
or just open `index.html` in Chrome, Firefox or Safari (everything loads from relative paths, no modules).
...
python3 tools/gen_atlas.py   # regenerate src/atlas-data.js (sprite trims, anchors, 9-slices); needs Pillow
```
...
| Pause / resume | Esc or P (Q on the pause screen quits to the title) |
...
- Synthesized audio (Web Audio): 23 effects plus a procedural music loop whose layers and tempo follow run intensity. No audio files exist in the pack.
...
Fixed 60 Hz timestep with an accumulator (max 5 steps per frame, then the backlog is dropped), rendering once per animation frame. All entities, particles, effect sprites and floating texts are pooled with swap-remove; the per-step hot path allocates nothing. Enemies use a 96px spatial hash for separation and the island ground (tiles, patches, shadows) is baked once into an offscreen canvas and blitted with a single draw. Sprites are drawn from precomputed per-frame trim boxes (generated by `tools/gen_atlas.py`) so mostly-transparent 192px frames do not waste fill rate, and everything off-screen is culled. Zoom snaps to half steps so the 2x pixel art stays on an even device-pixel grid.
...
`tools/playtest.mjs` launches headless Chrome through the DevTools Protocol using only Node's built-in `fetch` and `WebSocket`, presses Enter with a real key event, turns on the in-game bot, runs a full ten-minute run at 6x speed with screenshots each game-minute, verifies the R-key restart, and fails on any exception, console error or failed request. On the shipped tuning the bot, which never learns to kite and only dodges the most obvious threats, reaches 8:00 and dies as the Warlord lands with the horde at its 100-enemy cap, with zero errors logged across the run and restart. Screenshots land in `playtest-shots/` after `npm test`.

## Known limitations

- Balance was tuned with the scripted bot plus screenshots, not with human hands; expect the numbers in `TS.CFG` (src/game.js) and `ETYPES` (src/entities.js) to need a pass.
- All sound is synthesized. Some effects may be sharp on headphones; M mutes.
...
Art: [Tiny Swords](https://pixelfrog-assets.itch.io/tiny-swords) free pack by Pixel Frog. Everything else (code, procedural audio, island generation) lives in this repo.
